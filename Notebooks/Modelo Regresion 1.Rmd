...
trainSet2$Gender <- as.integer(trainSet2$Gender)-1
plot(trainSet2$Gender, trainSet2$class)
# To plot the estimated probability first define a vector x that
# takes values in the range of "gender"
z <- seq(from = min(trainSet2$Gender), to=max(trainSet2$Gender))
# obtain estimated coefficients using the coef() function
hat.beta.g <- coef(log.model_og)
lines(z, (1 + exp(-hat.beta.g[1] - hat.beta.g[2]*z))^(-1), col="blue")
abline(h=0.5, col="green")
# Plot partition of input space
# (straight "v"ertical line)
abline(v=-hat.beta.g[1]/hat.beta.g[2], col="red")
```
```{r}
