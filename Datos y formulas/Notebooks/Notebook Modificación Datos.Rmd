...
source("Rfunctions.R")
# Load the data
Default <- read.csv("stackoverflow_full.csv")
...
missing_values <- colSums(is.na(Default))
...
for (i in 1:nrow(Default)) {
  ...
  for (skill_name in names(top_skills)) {
    Default[i, skill_name] <- as.integer(check_skill(Default[i, "HaveWorkedWith"], top_skills[[skill_name]]))
  }
}

top_skillslist <- c("SQL","python", "java", "c_plus_plus", "cloud")
topskills_list <- Default %>% select(all_of(top_skillslist))

Default$SQL <- as.factor(Default$SQL)
Default$python <- as.factor(Default$python)
Default$java <- as.factor(Default$java)
Default$c_plus_plus <- as.factor(Default$c_plus_plus)
Default$cloud <- as.factor(Default$cloud)
```

```{r}
#convertir pais en continente
unique_countries <- unique(Default$Country)
unique_countries
...
Default$Continent <- sapply(Default$Country, segment_country)
```

```{r}
# Obtener el conteo de los continentes
continent_counts <- table(Default$Continent)
...
# numero de genero 
gender_counts <- table(Default$Gender)
...
```{r}
Default <- subset(Default, Default$Gender != "NonBinary")
Default$Gender <- as.factor(Default$Gender)
...
for (col in names(Default)) {
  if (is.character(Default[[col]])) {
    Default[[col]] <- as.factor(Default[[col]])
  }
}

Default$Employment <- as.factor(Default$Employment)
Default$Employed <- as.factor(Default$Employed)
Default$HaveWorkedWith <- as.character(Default$HaveWorkedWith)
