...
```
```{r}
hist(tfg$PreviousSalary, breaks = 20, col="lightblue", main="PreviousSalary", xlab="")
hist(tfg$ComputerSkills, breaks = 20, col="lightpink", main="ComputerSkills", xlab="")
hist(tfg$YearsCode, breaks = 20, col="orange", main="YearsCode", xlab="")
hist(tfg$YearsCodePro, breaks = 20, col="darkorange", main="YearsCodePro", xlab="")
```
```{r}
...
```
```{r}
plot(tfg$ComputerSkills, tfg$YearsCode, col=tfg$Gender)
plot(tfg$ComputerSkills, tfg$YearsCodePro, col=tfg$Employed)
```
```{r}
...
T <- table(tfg$EdLevel,tfg$Employed)
T
...
T <- table(tfg$Continent,tfg$Employed)
T
...
# Create a Boolean variable that is TRUE when defaulted="No"
# (this choice is important as will be discussed later)
y <- tfg1$Employed == 1 #ESTAMOS CONSIDERANDO LA VARIABLE EMPLOYED (CONTRATADO)
...
```{r}
setwd("~/Documents/GitHub/tfg/Datos y formulas")
write.csv(tfg1, "~/Documents/GitHub/tfg/Datos y formulas/datos_logit.csv", row.names = FALSE)
```
